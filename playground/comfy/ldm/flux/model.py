...
class Flux(nn.Module):
    ...
    def forward_orig(
        ...
        teacache = transformer_options.get("teacache")
        teacache_should_calc = True
        teacache_ori_img = None
        if teacache is not None and timestep_zero_index is None and control is None and not self.params.global_modulation and len(self.double_blocks) > 0 and getattr(self.double_blocks[0], "modulation", False):
            ...
            teacache_should_calc = teacache.should_calculate(modulated_inp, timesteps)
            if not teacache_should_calc and teacache.has_residual():
                img = img + teacache.previous_residual.to(device=img.device, dtype=img.dtype)
                img = self.final_layer(img, vec_orig)  # (N, T, patch_size ** 2 * out_channels)
                return img
            teacache_ori_img = img.detach().clone()
        ...
        if teacache is not None and teacache_should_calc and teacache_ori_img is not None:
            teacache.update_residual(img, teacache_ori_img)
