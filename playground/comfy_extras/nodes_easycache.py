...
def easycache_sample_wrapper(executor, *args, **kwargs):
    ...
    try:
        ...
        skipped = easycache.total_steps_skipped
        easycache.last_total_steps = total_steps
        easycache.last_steps_skipped = skipped
        original_easycache = orig_model_options["transformer_options"].get("easycache")
        if original_easycache is not None:
            original_easycache.last_total_steps = total_steps
            original_easycache.last_steps_skipped = skipped
        # catch division by zero for log statement; sucks to crash after all sampling is done
        try:
            speedup = total_steps/(total_steps-skipped)
        except ZeroDivisionError:
            speedup = 1.0
        logging.info(f"{easycache.name} - skipped {skipped}/{total_steps} steps ({speedup:.2f}x speedup).")
        easycache.reset()
